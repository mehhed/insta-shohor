...
// function for every single post discription 
const displayContent = (text) => {
    return text.length < 30 ? text : text.slice(0, 30) + "<span class='fw-bold'>... read more</span>";
};
// this arrow function for switc tab 
const switchTab = (id) => {
    if (id === "posts") {
...
// function for create single post 
                  ...
                  <div class="post__description">
                    <small>
                      <a class="post__name--underline" href="#">
                          ${(post.description.length < 40) ? post.description : post.description.slice(0, 40)} 
                      </a>
                    </small>
                  </div>
...
// function for showing post in display 
const showPosts = (posts) => {
    const productsContainer = document.getElementById( "posts" );
    productsContainer.innerHTML = "";
    // forEeach for get every single post data  
    posts.forEach((post) => {
      console.log(post); 
